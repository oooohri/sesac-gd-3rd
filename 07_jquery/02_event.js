...
// todo mouseover 이벤트 -> 해당 요소에 마우스가 올라갔을 때 실행(계속 유지)
$('.container').mouseover(() => {
    console.log('마우스 올라감');
});

// todo mouseout 이벤트 -> 해당 요소에서 마우스가 나갔을 때 실행
$('.container').mouseout(() => {
    console.log('마우스 나감');
    // 사용예시 -> 신세계 카테고리 텝이 열려있다가 마우스가 카테고리 영역을 벗어나면 mouseout이벤트로 사라지게함!
});

// todo hover 이벤트 -> css에서 더 많이 작성(사용)
...
// todo keydown 이벤트 : 선택한 요소에서 키보드를 누르는 이벤트가 발생할 경우
// 키보드를 누르고 있는 동안에도 계속 이벤트 발생
// 실시간 입력을 처리하거나 특수 키 (ex, 화살표 키, enter 키 등)를 감지하는데 사용

// todo keyup 이벤트 : 선택한 요소에서 키보드를 떼는 이벤트 발생할 경우
...
//     console.log('e.key', e.key); // 입력한 키 값 1개

//     if (e.key === 'Enter') {
//         console.log('엔터를 입력했습니다.');
//         console.log(e.target.value); // 로그인 로직, 검색 로직에서 사용할 수 있음
//     }
// });

// todo keyup 이벤트
// on() : 이벤트를 걸겠다. addEventListener과 비슷
