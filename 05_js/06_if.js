...
// 값이 정확히 정해져 있을때 switch 사용하는데, 실무에서 많이 사용하지는 않는다.
/*
...
// case에도 조건이 아닌 값을 작성
switch (x) {
  // case "admin": 과 같이 텍스트값이 들어갈수도 있다.
  case 3:
    console.log("x는 3입니다.");
    break;
  case 4:
    ...
    break;
    console.log("break 이후에 작성한 코드는 실행되지 않는다.");
  case 5:
    console.log("X는 5입니다.");
    break;
  default:
...
// if문으로 작성한 성적 계산하는 프로그램 switch문 사용하기
// switch (number) {
//   case 100:
//     console.log("A");
//     break;
...
//     ...
// }
// 위처럼 작성하는 경우 100, 90, ... 이런값이 아니면 원하는 동작을 하지 않음
...
//   삼항연산자는 이렇게 중첩하지 않는것을 추천한다.
isLoggedIn === true
  ? number > 10
    ? "로그인한 유저인데, number값이 10보다 큼"
    : "로그인한 유저인데, number값이 10보다 작음"
  : "로그인하지 않은 유저";
...
console.log(now);
now < 12
  ? console.log("지금 시각은 오전입니다")
  : console.log("지금 시각은 오후입니다");
