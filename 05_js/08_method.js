...
// 메서드는 자바스크립트에서 만들어놓은 함수이다. -> 그래서 뒤에 괄호가 붙음
...
// - indexOf() : 문자열에서 주어진 인자로 받은 문자열의 인덱스를 반환, 없다면 -1 반환
//  - replace(교체될 값, 대신들어갈 값) : 교체될 값의 문자를 대신 들어갈 값으로 변환 -> 제일 처음에 만난 문자만 변환
// - replaceAll() : 해당하는 모든 문자를 교체
// - slice(startIdx , [endIdx]) : startIdx부터 endIdx -1 까지 자름 (음수 인덱스 가능)
// - repeat(n) : 문자열을 n번 반복
// - split() : 인자로 받은 문자열을 기준으로 해당 문자열을 나누고, 나눈 문자열을 배열로 만들어 반환
// + - charAt(n) : 인자로 받은 숫자 n 인덱스 번호에 대한 문자열 하나 반환
let fruit = "applemango";

console.log(fruit.indexOf("apple")); // 0 -> 가장 첫번째 글자인 a가 걸린다. 대신 apple이라는 글자를 충족해야함!
...
let msg2 = "WoW! It's so amazing!!! WoW!!! "; // \' 사용하여 홑따옴표 표현가능
...
// includes() : 해당 배열에 인자로 받은 값과 동일한 요소가 있는지 확인 후 t/f 반환
...
console.log("for of를 사용한 배열에서의 반복 - 알파벳");
...
console.log(newArr2);
console.log(arr2); // filter는 원래 배열에 영향을 주지 않음.
