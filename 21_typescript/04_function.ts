...
// jsPrint("1", "2");
// jsPrint(["1", "2"], [1, 2]);
// jsPrint("두번째 인자는 안보냄"); // js에서는 문제 없이 동작
// ts에서는 에러 발생
...
const myfunc2 = (a: number): void => {
  console.log("함수 표현식으로 직상한 함수");
};

// interface에서 함수
interface Greet {
  name: string;
  hi(): void; // hi함수는 호출 시 hi라는 문자열을 콘솔에 찍고, 리턴 값 x
  // bye 함수는 문자열을 매개변수로 받고 해당 문자열을 리턴해 주기 때문에 함수의 타입이 string으로 작성
...
// never
console.log("------never--------");
// 아래의 코드 never 할당 불가 -> a의 인자값에 따라 break 할 수 있기 때문
// function neverEnd(a: number): never {
//   while (true) {
//     console.log("안끝나요");
//     if (a > 5) break; // 인자로 받은 a가 5를 초과한다면 함수 멈춤 -> 무한루프 아님! -> never작성 불가
//   }
// }
// neverEnd(1);

// 함수의 끝에 절대 도달하지 않는 경우에만 never할당 가능!
// -> 즉, 무한 루프
...
// 함수의 타입 (리턴 값이 있다면 해당 리턴값의 타입)은 인자를 받는 소괄호 뒤에 작성
// 만약 함수에 리턴값이 없다면 void타입
