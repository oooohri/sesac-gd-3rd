...
export default function UseAllState() {
  // useSelector를 통해 rootReducer에 관리되는 isLoggedIn state에 접근
...
function Child() {
  // index.js에서 저장한 count 가져옴
  const count = useSelector((state) => state.count);
  // useSelector 사용해서 store에 관리되는 전역 상태를 가져올건데, store의 state중 isLoggedIn의 값을 가져오겠다.
  const isLoggedIn = useSelector((state) => state.isLoggedIn);
  const dispatch = useDispatch();
  return (
    <div style={{ border: "5px solid skyblue", padding: "10px" }}>
      {/* count state보여주고 +1, -1,reset하는 버튼 만들기 */}
