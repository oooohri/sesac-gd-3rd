...
todoForm.addEventListener("submit", (e) => {
  ...
  // intput의 value 변수에 저장
  const todo = todoInput.value;

  //   const newTodo = document.createElement("li");
  //   // newTodo.innerText = todo;
  //   newTodo.append(todo);
  //   todos.append(newTodo); // ul태그에 todos에 newTodo li태그 추가.
  ...
  if (todo !== "") {
    const newTodo = document.createElement("li");
    //   newTodo.innerText = todo;
    // newTodo.append(todo); // newTodo는 li창. 즉 input창에 입력한 걸 li에 추가
    todos.append(newTodo); // 화면창에 li추가한걸 ul에 또 추가함.
